  ...
  for (i=0; s[i] != 0; i++)
    {
      if (s[i] == c) 
	{
	  num++; //adds one everytime the character appears
	}
    }
...
void my_strupper(char * s)
{
  ...
  for (i = 0; s[i] != 0; i++)
  {
    s[i] = toupper(s[i]); //converts to upper 
  }
...
void my_strlower(char * s)
{
 ...
 for (i = 0; s[i] != 0; i++)
 {
   s[i] = tolower(s[i]); //converts to lowercase
 }
...
void my_strcpy(char * s1, const char * s2)
{
  ...
  for (i = 0; s2[i] != 0; i++)
  {
    s1[i] = s2[i];  //copies each individual character
  }
  s1[i] = '\0'; //inserts null character
...
void my_strncpy(char * s1, const char * s2, int num)
{
  ...
  for (i = 0; i < num; i++)
  {
    s1[i] = s2[i]; //copies individual characters 
  }
  s1[i] = '\0';
...
void my_strcat(char * s1, const char * s2) 
{
   ...
   len = my_strlen(s1); //calls function for length of string 
   
   for (i = 0; s2[i] != 0; i++)
   {
     s1[i + len] = s2[i]; //appends string 
   }
...
void my_strncat(char * s1, const char * s2, int num)
{
   ...
   for (i = 0; i < num; i++)
   {
     s1[i + len] = s2[i]; //appends portion of string 
   }
...
{   
  ...
  len1 = my_strlen(s1); //length of string 1 
  len2 = my_strlen(s2); //lenth of S2 
  
  while (i < len1) //loops through s1 
  {
    if (s1[i] == s2[0])
    {
      start = i;
