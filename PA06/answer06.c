...
struct Image * loadImage(const char* filename)
{
  ...
  int retval;
  retval = fread(&hdr1, sizeof(struct ImageHeader), 1, fptr);
  ...
  if (img -> comment[hdr1.comment_len] != '\0')
  {
    //printf("\nNull Byte\n");
    return NULL;
  }
  ...
  retval = fread(img -> comment, sizeof(char), hdr1.comment_len, fptr);
  ...
  retval = fread(img -> data, sizeof(uint8_t), hdr1.width * hdr1.height, fptr);
